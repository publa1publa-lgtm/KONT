...
const chipIconOnly =
  "group flex h-7 w-7 shrink-0 items-center justify-center rounded-md p-0 transition-[background,color,transform] duration-150 touch-manipulation focus:outline-none focus-visible:ring-2 focus-visible:ring-[var(--studio-nav-active-ring)] active:scale-[0.96] disabled:pointer-events-none disabled:opacity-45";
...
const toneStyles: Record<ActionTone, { chip: string; icon: string; label: string; iconOnly: string }> = {
  default: {
    ...
    iconOnly: "text-[var(--muted)] hover:bg-[var(--studio-nav-hover)] hover:text-[var(--fg)]",
  },
  muted: {
    chip: "text-[var(--fg)] hover:bg-[var(--studio-nav-hover)] hover:shadow-[inset_0_0_0_1px_var(--studio-nav-active-ring)]",
  ...
  danger: {
    ...
    iconOnly: "text-[var(--ember)]/80 hover:bg-[var(--ember)]/12 hover:text-[var(--ember)]",
  },
};
...
function ActionChip({
  ...
  if (iconsOnly) {
    return (
      <button
        type="button"
        className={[chipIconOnly, s.iconOnly].join(" ")}
        onClick={onClick}
        disabled={disabled}
        aria-label={label}
...
export function ContentTableActions({
  ...
  return (
    <div
      className={[
        "studio-table-actions relative z-[1] inline-flex w-fit shrink-0 items-stretch",
        iconsOnly
          ? "studio-table-actions--icons gap-0.5"
          : compact
            ? "gap-0.5 rounded-2xl border border-[var(--line)]/90 bg-[var(--studio-surface-3)]/70 p-0.5 shadow-[inset_0_1px_0_rgba(255,255,255,0.05),0_10px_32px_-28px_rgba(0,0,0,0.35)] backdrop-blur-[2px]"
            : "rounded-2xl border border-[var(--line)]/90 bg-[var(--studio-surface-3)]/70 p-1 shadow-[inset_0_1px_0_rgba(255,255,255,0.05),0_10px_32px_-28px_rgba(0,0,0,0.35)] backdrop-blur-[2px]",
      ].join(" ")}
    ...
    >
      ...
      <span
        className={
          iconsOnly
            ? "mx-0.5 w-px shrink-0 self-stretch bg-[var(--line)]/70"
